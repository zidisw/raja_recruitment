<div class="flex flex-col gap-5">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div class="flex gap-2">
            <flux:button wire:click="exportCsv" variant="ghost" icon="document-arrow-down" class="w-full md:w-auto">
                {{ __('Export CSV') }}
            </flux:button>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-3">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <div class="min-w-36">
                <x-custom-select wire:model.live="filterStage" placeholder="{{ __('All Stages') }}" :options="['' => __('All Stages')] + collect($allStages)->mapWithKeys(fn($stage) => [$stage->value => $stage->label()])->toArray()" />
            </div>
            @if ($tab === 'riwayat')
                <div class="min-w-36">
                    <x-custom-select wire:model.live="filterStatus" placeholder="{{ __('All Statuses') }}" :options="['' => __('All Statuses')] + collect($statusOptions)->mapWithKeys(fn($status) => [$status->value => $status->label()])->toArray()" />
                </div>
            @endif
    ...
    <div class="flex items-center justify-start">
        <flux:button wire:click="toggleSelectionMode" variant="ghost" icon="check-circle" size="sm"
    ...
    @if($selectionMode && count($selectedIds) > 0)
        <div
            class="flex flex-wrap items-center gap-3 bg-blue-50/50 dark:bg-blue-900/20 p-3 rounded-xl border border-blue-100 dark:border-blue-800/30">
            <span class="text-sm font-medium text-blue-700 dark:text-blue-400">{{ count($selectedIds) }}
                {{ __('kandidat terpilih') }}</span>
            @if ($tab === 'administrasi')
                ...
                <flux:button size="sm" variant="danger" class="btn-danger-glow" @click="$dispatch('confirm-action', {
            ...
            @elseif ($tab === 'on-progress')
                <flux:button size="sm" variant="primary" icon="chevrons-up-down" wire:click="openBulkStageModal">
                    {{ __('Ubah Tahap') }}
                </flux:button>
                <flux:button size="sm" variant="danger" class="btn-danger-glow" @click="$dispatch('confirm-action', {
                                                title: 'Tolak Kandidat Terpilih?',
                                                description: '{{ count($selectedIds) }} kandidat akan ditandai sebagai tidak lolos rekrutmen.',
                                                variant: 'danger',
                                                method: 'bulkReject',
                                                confirmLabel: 'Ya, Tolak Semua'
                                            })">{{ __('Tolak') }}</flux:button>
            @elseif ($tab === 'riwayat')
                ...
                @if (auth()->user()->isSuperAdmin())
                    <flux:button size="sm" variant="danger" class="btn-danger-glow" @click="$dispatch('confirm-action', {
                                                                title: 'Hapus Kandidat Terpilih dari Riwayat?',
                                                                description: '{{ count($selectedIds) }} data kandidat akan dihapus permanen. Aksi ini tidak dapat dibatalkan.',
                                                                variant: 'danger',
                                                                method: 'bulkDeleteApplications',
                                                                confirmLabel: 'Ya, Hapus Semua'
                                                            })">{{ __('Hapus Terpilih') }}</flux:button>
                @endif
            @endif
            ...
            <table class="w-full text-sm modern-table">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800 bg-white dark:bg-zinc-900">
                    @forelse ($applications as $application)
                            ...
                            <td class="px-6 py-4 text-center">
                                @if ($tab === 'administrasi')
                                    <div class="flex flex-col items-center gap-1">
                                        ...
                                        <span
                                            class="text-xs text-zinc-400">{{ $application->recruitment_stage->label() }}</span>
                                    </div>
                                @elseif ($tab === 'on-progress')
                                ...
                                @else
                                    @if ($isTerminal && !auth()->user()->isSuperAdmin())
                                        <flux:badge
                                            color="{{ $application->recruitment_stage === \App\Enums\RecruitmentStage::HIRED ? 'green' : 'red' }}"
                                            size="sm">
                                    ...
                                    @else
                                        ...
                                        <x-custom-dropdown align="right" width="w-48">
                                            <x-slot name="trigger">
                                                <div
                                                    class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border cursor-pointer {{ $currentStage['badge'] }}">
                                            ...
                                            <x-slot name="content">
                                                <div class="py-1">
                                                    @foreach($stageConfig as $key => $cfg)
                                                        ...
                                                        @endphp
                                                        <button type="button" @if(!$isCurrent) @click="$dispatch('confirm-action', {
                                                            title: '{{ $title }}',
                                                            description: '{{ $desc }}',
                                                            variant: '{{ $variant }}',
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap w-px">
                                <div class="inline-flex flex-nowrap items-center justify-center gap-2 whitespace-nowrap">
                                    @if ($tab === 'administrasi')
                                        ...
                                        <flux:button size="sm" variant="ghost" class="text-red-500 btn-danger-glow" @click="$dispatch('confirm-action', {
                                                                    title: 'Tolak Kandidat?',
                                    ...
                                    @elseif ($tab === 'riwayat' && auth()->user()->isSuperAdmin())
                                        <flux:button size="sm" variant="ghost" icon="trash" class="text-red-500 btn-danger-glow"
                        ...
                        @if ($isExpanded)
                            <tr wire:key="candidate-{{ $application->id }}-expanded" wire:transition.opacity.duration.200ms
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $selectionMode ? 8 : 7 }}" class="px-6 py-8 text-center text-zinc-400">
    ...
    <flux:modal wire:model="showBulkStageModal" class="w-full max-w-xl">
        @php
            $bulkStages = $tab === 'on-progress'
                ? array_merge($progressStages, [\App\Enums\RecruitmentStage::HIRED, \App\Enums\RecruitmentStage::REJECTED])
                : $allStages;

            $bulkStageOptions = ['' => __('Pilih tahap...')];
            foreach ($bulkStages as $stage) {
                $bulkStageOptions[$stage->value] = $stage->label();
            }
        @endphp

        <div class="space-y-4">
            <flux:heading size="lg">{{ __('Ubah Tahapan Kandidat Terpilih') }}</flux:heading>
            <flux:subheading>
                {{ __('Terapkan perubahan tahap untuk :count kandidat.', ['count' => count($selectedIds)]) }}
            </flux:subheading>

            <flux:field>
                <flux:label>{{ __('Tahap Baru') }}</flux:label>
                <x-custom-select wire:model.live="bulkStage" :options="$bulkStageOptions" :searchable="true" />
                <flux:error name="bulkStage" />
            </flux:field>

            <div class="flex justify-end gap-3">
                <flux:button type="button" variant="ghost" wire:click="$set('showBulkStageModal', false)">
                    {{ __('Batal') }}
                </flux:button>
                <flux:button type="button" variant="primary" :disabled="!$bulkStage" @click="$dispatch('confirm-action', {
                        title: 'Ubah Tahapan Kandidat Terpilih?',
                        description: 'Tahapan akan diperbarui untuk {{ count($selectedIds) }} kandidat terpilih.',
                        variant: 'warning',
                        method: 'bulkUpdateStage',
                        confirmLabel: 'Ya, Ubah'
                    })">{{ __('Terapkan') }}</flux:button>
            </div>
        </div>
    </flux:modal>
        ...
        <div class="space-y-4">
            ...
            <form wire:submit="saveInterview" class="space-y-4">
                ...
                <div class="flex justify-end gap-3 mt-2">
                    <flux:button type="button" variant="ghost" wire:click="$set('showScheduleModal', false)">
                        {{ __('Batal') }}
                    </flux:button>
