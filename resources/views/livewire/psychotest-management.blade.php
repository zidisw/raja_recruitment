            ...
            <table class="w-full text-sm modern-table">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800 bg-white dark:bg-zinc-900">
                    @forelse ($applications_paginated as $app)
                        <tr wire:key="psychotest-row-{{ $app->id }}">
                            ...
                            <td class="px-6 py-4 text-center">{{ $app->psychotest?->test_date?->format('d M Y') ?? '—' }}
                            </td>
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap w-px">
                                ...
                                @else
                                    <flux:button size="sm" variant="ghost" wire:click="openCreate({{ $app->id }})"
                                        wire:target="openCreate({{ $app->id }})" icon="plus" class="app-action-btn">
                                        {{ __('Tambah') }}
                                    </flux:button>
                                @endif
                            </td>
            ...
            <form wire:submit="save" class="space-y-4">
                ...
                <flux:field>
                    ...
                    <div class="space-y-3">
                        @if($editingId && $app_psychotest = $this->currentPsychotest)
                            ...
                            @endif
                        @endif
                        <flux:input type="file" wire:model="psychotest_file"
                            wire:key="psychotest-file-{{ $editingId ?? 'new' }}" accept=".pdf" />
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <flux:button type="submit" variant="primary" wire:loading.attr="disabled"
                        wire:target="save,psychotest_file">
