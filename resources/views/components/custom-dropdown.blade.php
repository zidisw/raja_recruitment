...
<div {{ $attributes->merge(['class' => 'relative']) }} x-data="{
    ...
    <template x-teleport="body">
        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div
                class="rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(255,255,255,0.05)] border border-white/40 dark:border-white/10 overflow-hidden {{ $contentClasses }}">
                {{ $content }}
            </div>
        </div>
    </template>
