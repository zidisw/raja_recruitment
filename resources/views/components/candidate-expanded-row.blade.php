...
<div x-data="{ show: false }" x-init="requestAnimationFrame(() => show = true)" x-show="show"
    ...
    @if($application->stageLogs->isNotEmpty())
        <div class="col-span-2 sm:col-span-3 lg:col-span-4 mt-4 border-t border-zinc-200 dark:border-zinc-700 pt-4">
            <span class="text-xs font-medium uppercase tracking-wide text-zinc-400 mb-3 block">{{ __('Riwayat Perubahan Status') }}</span>
            <div class="space-y-4">
                @foreach ($application->stageLogs->sortByDesc('created_at') as $log)
                    <div class="relative pl-4 border-l-2 {{ $log->decision === 'rejected' ? 'border-red-400' : 'border-blue-400' }}">
                        ...
                        <p class="text-sm font-medium text-zinc-800 dark:text-zinc-200">
                            ...
                            <span class="font-normal text-xs ms-1 px-1.5 py-0.5 rounded {{ $log->decision === 'rejected' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' }}">{{ ucfirst($log->decision) }}</span>
                        </p>
